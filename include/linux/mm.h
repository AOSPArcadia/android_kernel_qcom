 ...
 */
#define CHP_VMA_NATIVE_STR "["
#define CHP_VMA_DALVIK_STR "]"
#define DALVIK_MAIN_HEAP "dalvik-main space (region space)"
#define NATIVE_HEAP "libc_malloc"

static inline const char *chp_decode_anon_name(const char *name) {
	if (name[0] == CHP_VMA_NATIVE_STR[0])
		return NATIVE_HEAP;
	if (name[0] == CHP_VMA_DALVIK_STR[0])
		return DALVIK_MAIN_HEAP;
	return name;
}
